...
// Returns earliest non-disabled date starting today; defaults to today
const getValidBaseDate = (
  ...
  for (let i = 0; i < MAX_DAYS; i++) {
    if (!disabled(potential)) return potential;
...
type DateTimeLocalInputProps = React.ComponentProps<typeof Calendar>;
...
const DateTimeLocalInput = ({
  ...
  const formateSelectedDate = React.useCallback(
    (selected: Date | undefined) => {
      ...
      const parsedDateTime = parseDateTime(selected);

      if (parsedDateTime) {
        // If no time has been selected yet, default to current device time
        const hasTime = Time && Time.includes(":") && Time.split(" ")[0].split(":").length === 2
        const now = new Date()
        const timeSource = hasTime ? Time : `${now.getHours()}:${now.getMinutes()}`
        parsedDateTime.setHours(
          parseInt(timeSource.split(":")[0]),
          parseInt(timeSource.split(":")[1]),
        );
        // If we auto-selected the current time, reflect it in state so the time list highlights correctly
        if (!hasTime) {
          const hour24 = now.getHours()
          const ampm = hour24 >= 12 ? "PM" : "AM"
          const hour12 = hour24 > 12 ? hour24 % 12 : hour24 === 0 || hour24 === 12 ? 12 : hour24
          onTimeChange(`${hour12}:${now.getMinutes()} ${ampm}`)
        }
        onValueChange(parsedDateTime);
      }
    },
